<?php
  /* --------------------------------------------------------------
   $Id: callback.php 15758 2024-02-29 16:35:54Z GTB $
...
if (is_file(DIR_FS_INC.'get_customers_status_by_id.inc.php')) {
...
if (is_array($request) && count($request) > 0) {
  ...
  if (xtc_db_num_rows($orders_query) > 0) {
    ...
    // update order
    if (is_file(DIR_FS_CATALOG.((defined('DIR_ADMIN')) ? DIR_ADMIN : 'admin/').'includes/modules/orders_update.php')) {
      ...
      include (DIR_FS_CATALOG.((defined('DIR_ADMIN')) ? DIR_ADMIN : 'admin/').'includes/modules/orders_update.php');
    } else {
      $customer_notified = 0;
      $comments = '';
      if (isset($request['type'])
